      ...
      let myRender = undefined
      {
        if (this.state.isText) {
          if (this.state.isEditable) {
            ...
            myRender = <div className="item">
              <span>{this.state.title}</span>
              <div className="item-text"><input type="text" placeholder={this.state.text} onChange={this.addNewText}></input></div> <div className="item-button"><button onClick={this.editField}>Guardar</button></div>
            </div>
          }
        } else {
